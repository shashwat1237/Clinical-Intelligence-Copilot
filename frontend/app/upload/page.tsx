...
export default function UploadPage() {
  ...
  // 🚨 FIXED: Global lock to prevent multiple simultaneous uploads
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setIsUploading(false); // Unlock on error so user can try again
    }
  };
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <main className="flex-1 p-8 overflow-y-auto flex items-center justify-center">
        <Card className="w-full max-w-lg p-8">
          ...
          <p className="text-sm text-gray-500 mb-6">Supported formats: text-based PDFs.</p>
          ...
          <div className="space-y-4">
            ...
            {/* Logic to show button only if NOT uploading, NOT polling, and NOT finished */}
            {!isPolling && !isReady && !isFailed && !isUploading && (
              <Button onClick={handleUpload} disabled={!file} className="w-full">
                Process Document
              </Button>
            )}

            {/* Combined UI: Shows Uploading spinner or backend Polling spinner */}
